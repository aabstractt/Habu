...
final class KnockbackVerticalCommand extends Argument {
    ...
    public function onPlayerExecute(Player $sender, string $label, array $args): void {
        if (count($args) < 2) {
            $sender->sendMessage(TextFormat::RED . 'Usage: /' . $label . ' vertical <name> <value>');
        ...
        $sender->sendMessage(Habu::prefix() . TextFormat::GREEN . 'Set vertical knockback for ' . $args[0] . ' to ' . $args[1] . '.');
